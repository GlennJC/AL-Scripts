...
function Install-MDTDhcp {
    <#
            ...
            4. Binds all available interfaces to listen for DHCP requests.
            5. If the server is a domain member, authorise the server in AD
    ...
    Invoke-LabCommand -ActivityName 'Installing and Configuring DHCP' -ComputerName $ComputerName -ScriptBlock {
        ...
        #FIX: 002 - Bind All Etherner Adapters to respond to DHCP Requests
        Get-NetAdapter | Set-DhcpServerv4Binding -BindingState $True | Out-Null

        #FIX: 003 - DHCP Authorisation in AD
...
function Import-MDTApplications {
    ...
    foreach ($App in $MDTApps.Applications.Application)
    {
        ...
        if ($App.ImportApp -eq "True") {
            ...
            {
                #Assume we will be importing the application
                $ContinueImport = $true
                #See if the file already exists
                $Uri = New-Object System.Uri($App.DownloadPath)
                $fileName = $Uri.Segments[$Uri.Segments.Count - 1]
                $Path = Join-Path -Path $downloadTargetFolder -ChildPath $fileName

                if (!(Test-Path -Path $Path))
                {
                    #File doesnt exist, attempt to download
                    try {
                        Get-LabInternetFile -Uri $App.DownloadPath -Path $downloadTargetFolder -ErrorAction Stop
                    }
                    catch {
                        Write-Error $_.Exception
                        Write-ScreenInfo "The app '$($App.Name)' could not be downloaded, skipping it."
                        $ContinueImport = $False
                    }
                }
                else {
                    Write-ScreenInfo "The app '$($App.Name)' is already downloaded."
                }
            }
            else {
                #No Download Path Specified (so check if the folder strructure is already present on disk
                if (-not (Test-Path -Path $downloadTargetFolder)){
                    #Error, couldnt locate the source directory, throw error
                    Write-ScreenInfo "Application '$($App.Name)' not located at $downloadTargetFolder, skipping it"
                    $ContinueImport = $False
                }
            }
...
if ($OperatingSystems)
{
    #We were passed a list of OSNames to install, so query these from the list of available Operating System Objects
    ...
    ForEach ($OperatingSystem in $OperatingSystems) {
        $OSParts = $OperatingSystem.Split("|")
        Switch ($OsParts.Count) {
            1 { 
                #Just the OperatingSystemName
                $os = $availableOperatingSystems | Where-Object {$_.OperatingSystemName -eq $OSParts} | Sort-Object -Property Version -Descending | Select-Object -First 1
                $osFriendlyName = $os -replace '[ \(\)]', ''
            }
            2 {
                #Name Plus Version
                $os = $availableOperatingSystems | Where-Object {$_.OperatingSystemName -like $OSParts[0] -and  $_.Version -eq $OSParts[1]} | Select-Object -First 1
                $osFriendlyName = $os -replace '[ \(\)]', ''
             }
            3 {
                #Name, Version, and Friendly Name
                $os = $availableOperatingSystems | Where-Object {$_.OperatingSystemName -like $OSParts[0] -and  $_.Version -eq $OSParts[1]} | Select-Object -First 1
                $osFriendlyName = $OSParts[2]
             }
        }
...
if ($CreateTaskSequences -eq 'True')
{
...
#Install and activate DHCP and bind to the Ethernet adapter.
