...
function Install-SCCM {
    ...
    (
        ...
        [string]$SccmInstallDirectory='C:\Program Files\Microsoft Configuration Manager'
    )
    ...
    if (-not (Test-Path -Path "$downloadTargetFolder\ADK1809")) {
        Write-LogFunctionExitWithError -Message "ADK Installation files not located at '$downloadTargetFolder\ADK'"
        return
    }

    if (-not (Test-Path -Path "$downloadTargetFolder\ADK1809WinPEAddons")) {
        Write-LogFunctionExitWithError -Message "ADK Windows PE Addons Installation files not located at '$downloadTargetFolder\ADKWinPEAddons'"
        return
    }
    ...
    #Bring all available disks online (this is to cater for any secondary drives)
    #Cant make the disk online and RW in the one command, need to perform two seperate actions
    ...
    $SCCMPreReqSourcesDirectory = Copy-LabFileItem -Path $SccmPreReqsDirectory -DestinationFolderPath C:\Install -ComputerName $SccmServerName -Recurse -PassThru
    ...
    Invoke-LabCommand -ActivityName 'Extend AD Schema' -ComputerName $SccmServerName -ScriptBlock {
        ...
        param
        (
            $SCCMSourcesDirectory
        )

        "$($SCCMSourcesDirectory)\SMSSETUP\BIN\X64\extadsch.exe"

    } -ArgumentList $SCCMSourcesDirectory
    ...
    Copy-LabFileItem -Path "$downloadTargetFolder\ADK1809" -DestinationFolderPath C:\Install -ComputerName $SccmServerName -Recurse

    Write-ScreenInfo "Copying ADK Windows PE Addons Install Files to server '$SccmServerName'..."
    Copy-LabFileItem -Path "$downloadTargetFolder\ADK1809WinPEAddons" -DestinationFolderPath C:\Install -ComputerName $SccmServerName -Recurse
   
    Write-ScreenInfo "Installing ADK on server '$SccmServerName'..." -NoNewLine
    $job = Install-LabSoftwarePackage -LocalPath C:\Install\ADK1809\adksetup.exe -CommandLine "/norestart /q /ceip off /features OptionId.DeploymentTools OptionId.UserStateMigrationTool OptionId.ImagingAndConfigurationDesigner" `
    -ComputerName $SccmServerName -NoDisplay -AsJob -PassThru
    Wait-LWLabJob -Job $job -NoDisplay

    Write-ScreenInfo "Installing ADK Windows PE Addons on server '$ComputerName'..." -NoNewLine
    $job = Install-LabSoftwarePackage -LocalPath C:\Install\ADK1809WinPEAddons\adkwinpesetup.exe -CommandLine '/norestart /q /ceip off /features OptionId.WindowsPreinstallationEnvironment' `
    -ComputerName $SccmServerName -NoDisplay -AsJob -PassThru
    Wait-LWLabJob -Job $job -NoDisplay
...
PrerequisiteComp=1
PrerequisitePath=$SCCMPreReqSourcesDirectory
...
"@
    ...
    $job = Install-LabSoftwarePackage -ComputerName $SccmServerName -LocalPath "$($SCCMSourcesDirectory)\SMSSETUP\BIN\X64\setup.exe" -CommandLine '/Script "C:\Install\ConfigMgrUnattend.ini" /NoUserInput' -AsJob -PassThru
    Wait-LWLabJob -Job $job -NoDisplay
}
